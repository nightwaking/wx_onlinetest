...
<?php if(is_array($kind)): $i = 0; $__LIST__ = $kind;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="weui-cells">
            <div class="weui-cell weui-cell_access">
                <div class="weui-cell__bd"><?php echo ($vo["choice_name"]); ?></div>
                <a class="weui-btn weui-btn_mini weui-btn_warn" href= "<?php echo U('Pyschology/choice_delete', array('id'=>$vo['cid'], 'sid'=>$sid));?>">删除选项</a>   
            </div>
        </div><?php endforeach; endif; else: echo "" ;endif; ?> 
<form action="<?php echo U('Pyschology/add_choice');?>" method="post">
        <div class="weui-cells__title">添加选项</div>
       	<?php $__FOR_START_466970454__=1;$__FOR_END_466970454__=5;for($i=$__FOR_START_466970454__;$i < $__FOR_END_466970454__;$i+=1){ ?><div class="weui-cells">
            <div class="weui-cell">
                <div class="weui-cell__bd">
                    <input class="weui-input" type="text" name="choice_<?php echo ($i); ?>" placeholder="请输入选择项<?php echo ($i); ?>">    
                </div>
            </div>
        </div><?php } ?>
        <input type="text" name="skill_id" value="<?php echo ($qid); ?>" hidden>
        <input type="text" name="s_id" value="<?php echo ($sid); ?>" hidden>
