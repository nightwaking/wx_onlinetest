...
<body ontouchstart>
    <div class="page__bd">
        ...
        <form action="<?php echo U('Common/skillDisplay');?>" method="post">
        ...
        <?php if(is_array($kind)): $i = 0; $__LIST__ = $kind;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="weui-cells">
            ...
            <?php $__FOR_START_1943000506__=1;$__FOR_END_1943000506__=4;for($i=$__FOR_START_1943000506__;$i < $__FOR_END_1943000506__;$i+=1){ ?><label class="weui-cell weui-check__label" for="(<?php echo ($vo["qid"]); ?>+<?php echo ($i); ?>)">
                <div class="weui-cell__bd">
                    <p class="show_answer"><?php echo ($i); ?></p>
                </div>
                <div class="weui-cell__ft">
                    <input type="radio" class="weui-check show_title" name="radio_<?php echo ($vo["qid"]); ?>" id="(<?php echo ($vo["qid"]); ?>+<?php echo ($i); ?>)" value="<?php echo ($i); ?>">
...
<script type="text/javascript">
    window.onload = function(){
        var showTitle = document.getElementsByClassName("show_title");
        var showAnswer = document.getElementsByClassName("show_answer");
        for (var i=0; i<showTitle.length; i++){
            var showValue = showTitle[i].value;
            if (showValue == 1){
                showAnswer[i].innerHTML = "未掌握";
            }else if(showValue == 2){
                showAnswer[i].innerHTML = "基本掌握";
            }else{
                showAnswer[i].innerHTML = "完全掌握";
            }
        }
    }
</script>
